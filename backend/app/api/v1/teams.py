    ...
    return [
        {
            ...
            "role_definitions": t.role_definitions,
...
async def create_team(
    ...
    return {
        ...
        "role_definitions": team.role_definitions,
...
@router.get("/{team_id}/role-definitions")
async def get_team_role_definitions(team_id: int, db: AsyncSession = Depends(get_db)):
    """Get a team's role definitions. Public endpoint."""
    team = await _get_team_or_404(db, team_id)
    return team.role_definitions or {}


ALLOWED_ROLE_KEYS = {"captain", "pm", "tech_lead", "student"}


@router.put("/{team_id}/role-definitions")
async def update_team_role_definitions(
    team_id: int,
    data: RoleDefinitionsUpdate,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Update a team's role display overrides. Requires captain/PM/admin/advisor permission."""
    team = await _get_team_or_404(db, team_id)
    ...
    # Validate role keys
    for key in data.role_definitions:
        if key not in ALLOWED_ROLE_KEYS:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"无效的角色键 '{key}'，允许的值为: {', '.join(sorted(ALLOWED_ROLE_KEYS))}",
            )

    # Merge with existing
    existing = team.role_definitions or {}
    for key, item in data.role_definitions.items():
        existing[key] = item.model_dump()
    team.role_definitions = existing
    await db.commit()
    return {"message": "角色定义已保存", "role_definitions": team.role_definitions}
...
async def update_team_member(
    ...
    if data.parent_member_id is not None:
        # Cycle detection
        if data.parent_member_id == tm.id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="不能将成员设为自己的上级")
        visited = {tm.id}
        current_parent_id = data.parent_member_id
        while current_parent_id is not None:
            if current_parent_id in visited:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="不能形成循环层级关系")
            visited.add(current_parent_id)
            parent_result = await db.execute(
                select(TeamMember).where(TeamMember.id == current_parent_id)
            )
            parent_tm = parent_result.scalar_one_or_none()
            if not parent_tm:
                break
            current_parent_id = parent_tm.parent_member_id
